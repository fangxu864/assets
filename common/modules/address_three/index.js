...
// 插件使用说明：
//本插件为省市区三地联动插件
//使用方法为p_c_a.province_city_area("div1","div2","div3")
//调用p_c_a的province_city_area方法然后传入三个容器的ID即可；
//p_c_a为province（省）city（市）area(区）的缩写
//获取省市区为获取p_c_a的三个属性，即p_c_a.province  p_c_a.city  p_c_a.area

/**使用示例
*p_c_a.province_city_area("div1","div2","div3")
*var btn=document.getElementById("btn")
*btn.onclick=function () {
*console.log(p_c_a.province+"-"+p_c_a.city+"-"+p_c_a.area)
*}
*/
...
//                startMove(_this.opt,{"height":0})
        ...
        var a=new SelectScroll({
            id:id1,
            arr:provinceArr,
            callback:function (cur_opt) {
                var _province=cur_opt



                var cityArr=[];
                //找省份函数
                function findProvince(cur_opt) {
                    for(var i=0;i<provinceList.length;i++){
                        if(provinceList[i]["name"]==cur_opt){
                            return i;
                        }
                    }
                }
                var provinceIndex=findProvince(cur_opt);
                if(provinceIndex>=31){
                ...
                for(var j=0;j<provinceList[provinceIndex].cityList.length;j++){
                    cityArr.push(provinceList[provinceIndex].cityList[j]["name"])
                }

                var b=new SelectScroll({
                    id:id2,
                    arr:cityArr,
                    callback:function (cur) {
                        ...
                        var areaArr=[]
                        //找区域函数
                        function findCity(cur) {
                            for(var i=0;i<provinceList[provinceIndex].cityList.length;i++){
                                if(provinceList[provinceIndex].cityList[i]["name"]==cur){
                                    return i;
                                }
                            }
                        }
                        var cityIndex=findCity(cur);



                        for(var j=0;j<provinceList[provinceIndex].cityList[cityIndex].areaList.length;j++){
                            areaArr.push(provinceList[provinceIndex].cityList[cityIndex].areaList[j])
                        }


                        var c=new SelectScroll({
                            id:id3,
                            arr:areaArr,
                            callback:function (c) {
                                var _area=c

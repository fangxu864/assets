...
var Main = PFT.Util.Class({
	...
	initImgUpload: function () {
		var uploader = new Fileupload({
			...
			loading: function (formControls) {
				PFT.Util.STip("success", '<p style="width:160px;">正在上传图片,请稍后</p>', 3000);
			},
	...
	renderActivityPage: function (data) {
		...
		$("#saveBtn").attr("data-spid",data.spid);
	...
	saveAddNewActivity: function (e) {
		...
		console.log(only_member)
		if (!title) return PFT.Util.STip("fail", "活动名称不为空!", 3000);
		if (!beginDate) return PFT.Util.STip("fail", "活动起始时间不为空!", 3000);
		if (!endDate) return PFT.Util.STip("fail", "活动截止时间不为空!", 3000);
		if (beginDate > endDate) return PFT.Util.STip("fail", "起始时间不能迟于截止时间!", 3000);
		if(!only_member) PFT.Util.STip("fail", "请选择是否仅限会员卡!", 3000);
		switch (type) {
			case '1':
	...
	getTypeAParam: function () {
		...
		var param;
		if (!couponId) return PFT.Util.STip("fail", "优惠券ID不为空!", 3000);
		if (!reg.test(couponNum)) return PFT.Util.STip("fail", "赠券张数在1~999999之间!", 3000);
		param = {
			coupon_id:couponId,
			relation_pid:"0" ,
	...
	getTypeBParam: function () {
		...
		if (!reg.test(red_pack_money)) return PFT.Util.STip("fail", "红包金额在0~10亿之间!", 3000);
		param = {
			coupon_id:"",
			relation_pid: "",
			coupon_num: "0",
	...
	getTypeCParam: function () {
		...
		if (!couponIdInp) return PFT.Util.STip("fail", "优惠券ID不为空!", 3000);
		if (!proInp) return PFT.Util.STip("fail", "活动产品不为空!", 3000);
	...
	saveReq: function (param) {
		...
		PFT.Util.Ajax('/r/product_Coupon/CreateShare', {
			...
			success: function (res) {
				if (res.code == 200) {
					PFT.Util.STip("success", "保存成功!", 3000);
					window.location.href = "marketing_share_list.html";
				} else {
					PFT.Util.STip("fail", res.msg, 3000);
				}
			}
		})
	},

...
var Distors = RichBase.extend({
	...
	fetch : function(){
		...
		api.getDistors({
			...
			search_type:$("#fs_select_box span.t").attr("search_type")
		})
	},
	show : function(){
		if(this.isAdmin) return false;
		// var search_type=$("#fs_select_box span.t").attr("search_type");
		// var allDistors = this.statics.allDistors;
		this.container.show();
		// if(!isEmptyObject(allDistors[search_type])){
		// 	this.container.html(this.render(allDistors[search_type]));
		// }else{
		// 	this.fetch();
		// }
		this.fetch();

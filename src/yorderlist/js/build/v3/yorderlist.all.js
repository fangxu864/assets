...
/***/ function(module, exports, __webpack_require__) {
	...
	//			return $("#cSelectTrigger").attr("data-city") || "";
		...
		initSearchItem : function(data){
			this.statics.Provice_City_Cache = data;
			...
			this.advanceSearchBox.children(".con").append(tmemesHtml+provsHtml);
		...
		onAdanceSearchItemClick : function(that,e){
			...
			if(!tarItem.hasClass("provice")) return false;
			var provid = tarItem.attr("data-provice");
			var cacheData = that.statics.Provice_City_Cache.provices;
			var con = $("#advanceSearchBox").children(".con");
			con.children(".cityLine").remove();
			if(!cacheData || !cacheData[provid]) return false;
			var citys = cacheData[provid]["citys"];
			if(!citys) return false;
			var citysHtml = that.builtSearchItem({
				title : "所在城市",
				param : "city",
				data : citys
			});
			con.append(citysHtml);
		},
	...
	//		$("#cityParamUl").children(".first").addClass("active").siblings().removeClass("active");
			$("#advanceSearchBox").find(".cityLine").remove();
	...
	//		this.pSelect.refresh();
		...
		builtSearchItem : function(opt){
			...
			if(data && Object.prototype.toString.call(data)=="[object Object]" && !PFT.Help.isEmptyObj(data)){
				html += '<div class="line '+param+'Line '+param+'"><div class="inCon"><div class="lt">'+title+'</div><ul id="'+param+'ParamUl" class="listUl '+param+'ParamUl">';
				html += '<li class="item first active '+param+'"><a href="javascript:;" data-'+param+'="">全部</a></li>';
				for(var i in data){
					var d = data[i];
					var name = d["name"];
					html += '<li class="item '+param+'" data-'+param+'="'+i+'"><a href="javascript:;"><span class="name">'+name+'</span></a></li>';
				}
				html += '</ul></div><a style="display:none" class="toggleBtn" href="javascript:;"><span class="t">更多</span><i class="iconfont down">&#xe673;</i><i class="iconfont up">&#xe695;</i></a></div>';
			}
			setTimeout(function(){
				that.fixMoreBtn($("#"+param+"ParamUl"));
			},200)
			return html;
		},
		fixMoreBtn : function(paramUl){
			var pWidth = paramUl.parent().width();
			var moreBtn = paramUl.parent().next();
			var totWidth = 0;
			var childs = paramUl.children();
			childs.each(function(){
				var tarItem = $(this);
				totWidth += tarItem.outerWidth();
			})
			if(totWidth>=pWidth){

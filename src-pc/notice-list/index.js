...
var Main = PFT.Util.Class({
    ...
    onTabItemClick : function(e){
        ...
        this.tabConContainer.children(".tabCon_"+type).addClass("active").siblings().removeClass("active");
    ...
    onPaginationBtnClick : function(e){
        ...
        var toPage = tarBtn.hasClass("next") ? (fromPage+1) : (fromPage-1);
                ...
                success : function(res){
                    ...
                    if(code==200){
                        if(data.length){
                            ...
                            Message.alert("没有更多了..");
                            that.renderPagination(type,toPage,"noMore");
                        }
                    }else{
                        Message.alert(msg);
                    }
    ...
    render : function(type,data){
        var listUl = this.tabConContainer.children(".tabCon_"+type).children(".listUl");
        if(!listUl.length) return false;
        console.log(data);

    },
    renderPagination : function(type,page,state){
        var pag = this.tabConContainer.children(".tabCon_"+type).children(".paginationBox");
        var prevBtn = pag.children(".prev");
        var nextBtn = pag.children(".next");
